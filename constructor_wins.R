...
teams <- c("Ferrari", "McLaren", "Williams", "Mercedes", "Red Bull", "Renault", "Alfa Romeo", "Racing Point")
...
plot_wins <- function(period)
{
  ...
  if(period == 2017)
  {
    winnings_constructor <- results_df[results_df$positionText == "1" & results_df$raceId >= 969 & results_df$raceId <= 1030,]$constructorId
    races_entered_constructors <-results_df[results_df$raceId >= 969 & results_df$raceId <= 1030,]$constructorId
  }
  else if(period == 2014)
  {
    winnings_constructor <- results_df[results_df$positionText == "1" & results_df$raceId >= 900 & results_df$raceId <= 968,]$constructorId
  ...
  {
    winnings_constructor <- results_df[results_df$positionText == "1" & results_df$raceId >= 18 & results_df$raceId <= 70,]$constructorId
    
    races_entered_constructors <- results_df[results_df$raceId >= 18 & results_df$raceId <= 70,]$constructorId
  }
  else if(period == 1998)
  {
    winnings_constructor <- results_df[results_df$positionText == "1" & results_df$raceId >= 71 & results_df$raceId <= 206,]$constructorId
...
barplot(table_wins_period, col=rainbow(5), 
        ...
        main = 'Колко победите има даден отбор през даден период',
